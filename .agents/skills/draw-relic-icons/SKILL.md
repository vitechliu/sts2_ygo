...
3. Resolve the read-only vanilla reference root from the active `AGENTS.md` instructions or `STS2_VANILLA_ROOT`, then inspect 3-6 relevant original icons in `${STS2_VANILLA_ROOT}/images/relics/`. Ask for the root if it is not configured. Choose references by similar object category, silhouette, or palette. Use them only to learn visual grammar; do not copy their subject, exact geometry, or distinctive details. Never modify that directory.
