...
router.post('/localization', async (req, res) => {
    try {
        const normalizedEnName = requireString(
            req.body.enName,
            'English name',
            { maxLength: 128 }
        );
        const cnName = requireString(
            req.body.cnName ?? '',
            'Chinese name',
        ...
        if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(normalizedEnName)) {
            throw new ValidationError('A valid English name is required for localization');
        }
...
router.post('/', async (req, res) => {
    try {
        ...
        const normalizedEnName = requireString(enName, 'English name', { maxLength: 128 });

        if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(normalizedEnName)) {
            throw new ValidationError('A valid English name is required');
        }
        ...
        const result = await runCards(
            `INSERT INTO cards (card_id, name, cn_name, en_name, types, description, atk, def, level, attribute, race, raw_data) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [normalizedCardId, name, cnName, normalizedEnName, types, description, atk, def, level, attribute, race, sanitizedRawData]
        );
