"""只读导入仪式依赖；显式源路径，保留嵌套 Prefab 覆盖、轨道偏移与曲线。"""
...
def main():
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--source',type=Path,required=True);ap.add_argument('--output',type=Path,default=Path('VYgo/scenes/summon/ritual'));args=ap.parse_args()
    source=args.source.resolve();root=source/'AssetBundles/Duel/Timeline/Summon/SummonRitual';out=args.output.resolve()
    if out.is_relative_to(source):raise ValueError('输出目录不得位于只读源工程内。')
    ...
    def asset(ref,mesh=False):
        ...
        if mesh and name not in colored_meshes:
            ...
            if channels[3]['dimension']:
                ...
                colored_meshes[name]=dict(vertices=[[x,y,-z] for x,y,z in channel(0)],normals=[[x,y,-z] for x,y,z in channel(1)],uv=[[u,1-v] for u,v in channel(4)],colors=channel(3),indices=[indices[i+j] for i in range(0,len(indices),3) for j in [0,2,1]])
        return name
    def material(ref):
        guid=ref.get('guid');p=paths.get(guid)
        if p is None:return None
        ...
        return guid
    stages={}
    for name,p in [('main',root/'SummonRitual01.prefab'),*[(f'show{i}',root/f'SummonRitualShowUnitCard/SummonRitualShowUnitCard{i:02}.prefab') for i in range(1,7)]]:
        ...
        timeline('',0,5.883333 if name=='main' else 1.333333)
        stages[name]=dict(nodes=nodes,tracks=tracks,events=events,duration=5.883333 if name=='main' else 1.333333)
    data=clean(dict(stages=stages,materials=mats,dependencies=deps,coloredMeshes=colored_meshes,warnings=warnings))
    (out/'ritual_source.json').write_text(json.dumps(data,ensure_ascii=False,separators=(',',':'))+'\n',encoding='utf-8')
